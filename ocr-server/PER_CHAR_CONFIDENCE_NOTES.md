...
Field này có độ dài luôn bằng `len(text)` và không phá cấu trúc response cũ.

## File đã sửa / thêm

- `app/main.py`
  - Import và gọi `apply_paddleocr_char_confidence_patch()` trước khi khởi tạo `PaddleOCR`.
  - Bật `return_word_box=True` trong config OCR và khi gọi `ocr.predict(...)`.
  - Thêm helper `normalize_char_confidences(...)`.
  - Thêm field `per_char_confidences` vào mỗi item trong `results`.
  - Thêm field `char_candidates` vào mỗi item trong `results` và thêm
    `candidates` trong từng phần tử `chars`.
...
- `scripts/test_per_char_confidences.py`
  ...
  - In `text`, `confidence`, `per_char_confidences`, `len(text)`, `len(per_char_confidences)`.
...
Có, khi patch lấy được `char_confidences`, các giá trị đến từ `conf_list` thật trong decoder của PaddleOCR/PaddleX.

Không phải số random hoặc tự bịa. Chỉ fallback bằng line confidence khi không lấy được `conf_list`.
